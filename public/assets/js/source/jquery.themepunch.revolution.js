...
"use strict";
	...
	jQuery.fn.extend({
        ...
        revresize: function(oy) {
            var c=jQuery(this);
            var opt = c[0].opt;
            containerSafeResized(c, opt);
        },
		// METHODE PAUSE
		revkill: function(oy) {
...
var containerSafeResized = function(c, opt) {
    if (jQuery('body').find(c)!=0) contWidthManager(opt);							
    ...
    if (c.outerWidth(true)!=opt.width || c.is(":hidden") || (hchange)) {
            opt.lastwindowheight = jQuery(window).height();
            containerResized(c,opt);
    }
}
